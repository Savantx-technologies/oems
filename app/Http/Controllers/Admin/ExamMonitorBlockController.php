...
{
    private function ensureSchoolAdmin()
    {
        if (auth('admin')->user()->role !== Admin::ROLE_SCHOOL_ADMIN) {
            abort(403, 'Only School Admin can manage monitoring blocks.');
        }
    }
    ...
    public function index($examId)
    {
        ...
        $exam = Exam::with([
            'monitorBlocks' => fn($q) => $q->where('assignee_type', Admin::class),
            'monitorBlocks.assignee',
            'monitorBlocks.attempts.user',
            'monitorBlocks.students',
            'attempts.user',
            'attempts.monitorBlock',
        ])->where('school_id', $admin->school_id)->findOrFail($examId);
        ...
        return view('admin.exams.monitor-blocks', compact('exam', 'assignableMonitors', 'rosterStudents', 'attemptsByUser'));
    }
    ...
    public function store(Request $request, $examId)
    {
        $this->ensureSchoolAdmin();
        $admin = auth('admin')->user();
        $exam = Exam::where('school_id', $admin->school_id)->findOrFail($examId);

        $request->validate([
            'name' => ['required', 'string', 'max:255'],
            'assignee_id' => [
                'nullable',
                Rule::exists('admins', 'id')->where(fn ($query) => $query->where('school_id', $admin->school_id)),
            ],
    ...
    public function moveStudent(Request $request, $examId, User $student)
    {
        ...
        abort_unless(
            ...
            && (int) $student->school_id === (int) $admin->school_id
    ...
    public function autoAssign(Request $request, $examId)
    {
        ...
        ExamMonitorBlock::where('exam_id', $exam->id)->where('assignee_type', Admin::class)->delete();
