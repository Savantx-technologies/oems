...
<div class="space-y-6">
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-bold text-gray-900">Monitoring Block Assignment</h1>
            <p class="text-sm text-gray-500 mt-1">{{ $exam->title }} | {{ $exam->school->name ?? 'Unknown School' }}</p>
        </div>
        <div class="flex gap-3">
            <a href="{{ route('superadmin.exams.show', $exam->id) }}" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">Exam Details</a>
            <a href="{{ route('superadmin.exams.index') }}" class="px-4 py-2 bg-gray-100 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-200">Back</a>
        </div>
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-5 shadow-sm">
            ...
            <p class="mt-1 text-sm text-gray-500">Roster available for this school and class.</p>
        </div>
        ...
        <div class="rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50 to-white p-5 shadow-sm">
            ...
            <p class="mt-1 text-sm text-gray-500">Monitoring groups available for delegation.</p>
        </div>
    ...
    <div class="grid gap-6 xl:grid-cols-3">
        <div class="xl:col-span-1 space-y-6">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                <div class="flex items-start justify-between gap-3 mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Split students into ready-to-use monitoring groups.</p>
                    </div>
                    <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">Fast Setup</span>
                </div>
                <form method="POST" action="{{ route('superadmin.exams.monitor-blocks.auto-assign', $exam->id) }}" class="space-y-4">
                    @csrf
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Assign Sub Super Admins Round-Robin</label>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                <div class="mb-4">
                    ...
                    <p class="text-sm text-gray-500">Create zone-based or supervisor-specific monitoring groups.</p>
                </div>
                <form method="POST" action="{{ route('superadmin.exams.monitor-blocks.store', $exam->id) }}" class="space-y-4">
                    @csrf
                    <div>
        ...
        <div class="xl:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                <div class="flex items-center justify-between gap-3 mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Keep assignments tidy and supervisors balanced.</p>
                    </div>
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white p-4 shadow-sm">
                        <form method="POST" action="{{ route('superadmin.exams.monitor-blocks.update', ['id' => $exam->id, 'block' => $block->id]) }}" class="space-y-3">
                            @csrf
                            @method('PUT')
                            <div>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Assigned Sub Super Admin</label>
                                <select name="assignee_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="">Unassigned</option>
                                    @foreach($assignableSubSuperAdmins as $monitor)
                                    <option value="{{ $monitor->id }}" {{ $block->assignee_id == $monitor->id && $block->assignee_type === \App\Models\SuperAdmin::class ? 'selected' : '' }}>
                                        {{ $monitor->name }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <form method="POST" action="{{ route('superadmin.exams.monitor-blocks.destroy', ['id' => $exam->id, 'block' => $block->id]) }}" class="mt-2">
                            @csrf
                            @method('DELETE')
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <form id="superadmin-bulk-assign-{{ $exam->id }}" method="POST" action="{{ route('superadmin.exams.monitor-students.bulk-move', ['id' => $exam->id]) }}" class="px-6 py-4 border-b bg-blue-50/60 flex flex-col lg:flex-row lg:items-center gap-3">
                    @csrf
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm text-left">
                        <thead class="bg-gray-50 text-gray-600 font-medium border-b">
                            <tr>
                                <th class="px-6 py-3">
                                    <input type="checkbox" onclick="document.querySelectorAll('.superadmin-bulk-student-checkbox').forEach(cb => cb.checked = this.checked)">
                                </th>
                                <th class="px-6 py-3">Student</th>
                                <th class="px-6 py-3">Current Block</th>
                                <th class="px-6 py-3">Status</th>
                                <th class="px-6 py-3">Reassign</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                <td class="px-6 py-3">
                                    <input form="superadmin-bulk-assign-{{ $exam->id }}" type="checkbox" name="student_ids[]" value="{{ $student->id }}" class="superadmin-bulk-student-checkbox rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                </td>
                                <td class="px-6 py-3">
                                ...
                                <td class="px-6 py-3">
                                    <form method="POST" action="{{ route('superadmin.exams.monitor-students.move', ['id' => $exam->id, 'student' => $student->id]) }}" class="flex items-center gap-2">
                                        @csrf
                                        @method('PUT')
                                        <select name="monitor_block_id" class="rounded-md border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                                            <option value="">Unassigned</option>
