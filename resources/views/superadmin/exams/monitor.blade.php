...
<div class="h-[calc(100vh-140px)] flex flex-col" x-data="monitorApp()">
    ...
    <div class="flex justify-between items-center mb-4 shrink-0">
        <div>
            ...
            <p class="text-sm text-gray-500">{{ $exam->title }} | Class {{ $exam->class }} | <span class="font-semibold">{{ $exam->school->name }}</span></p>
        </div>
        <div class="flex gap-3">
            ...
            <a href="{{ route('superadmin.exams.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">Exit</a>
        </div>
    </div>
    ...
    return {
        ...
        terminate(id) {
            ...
            fetch(`/superadmin/attempts/${id}/terminate`, {
                ...
                body: JSON.stringify({ reason: 'SuperAdmin Manual Action' })
            })
            .then(res => res.json())
            .then(data => {
...
function studentCard() {
    return {
        ...
        async startCamera(studentId) {
            ...
            try {
                const res = await fetch(`/superadmin/attempts/${studentId}/stream`);
                ...
                // For SuperAdmin viewing, we might not need to send an answer back if the student is just broadcasting,
                // but typically WebRTC requires an answer to establish the connection.
                // However, the current implementation in Admin/LiveMonitorController doesn't seem to save the answer from the viewer side 
                // back to the DB for the student to pick up (it saves 'answer' in sendSignal but that's usually for the student sending an answer to an offer).
                // Assuming the student is the Offerer and Admin is the Answerer.
                
                // NOTE: The provided Admin implementation suggests a specific signaling flow. 
                // Since I cannot see the student-side JS, I will assume the Admin implementation is correct and replicate it.
                // But wait, the Admin JS sends 'answer' to `attempts/{id}/signal`.
                
                await fetch(`/superadmin/attempts/${studentId}/signal`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                ...
                this.iceInterval = setInterval(async () => {
                    ...
                    const r = await fetch(`/superadmin/attempts/${studentId}/stream`);
