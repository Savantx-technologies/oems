...
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        ...
        <form action="{{ route('student.notifications.markRead') }}" method="POST" class="flex-shrink-0">
            @csrf
            <button type="submit"
    ...
    <div class="bg-white/80 backdrop-blur shadow-lg rounded-2xl border border-gray-100 overflow-hidden">
        <div class="">
            ...
            @php
            $isExam = $notification->type === 'exam';
            $examId = $notification->data['exam_id'] ?? null;
            $exam = $isExam ? ($exams[$examId] ?? null) : null;
            @endphp
            <div class="flex flex-col sm:flex-row items-start gap-5 px-6 py-6 sm:py-7 group transition-colors {{ $notification->is_read ? 'bg-gray-50/40' : 'bg-white hover:bg-indigo-50/30' }} border-b last:border-b-0 border-gray-100">
                ...
                <div class="flex-shrink-0 w-14 h-14 rounded-xl flex items-center justify-center shadow-sm {{ $notification->is_read ? 'bg-gray-200 text-gray-400' : 'bg-indigo-100 text-indigo-600' }}">
                    <i class="bi {{ $isExam ? 'bi-calendar-event' : 'bi-bell' }} text-2xl"></i>
                </div>
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="mt-4 flex flex-wrap items-center gap-2">
                        @if($exam)
                        <a href="{{ route('student.notifications.readAndRedirect', $notification->id) }}"
                        ...
                        @if(!$notification->is_read)
                        <form action="{{ route('student.notifications.markSingleRead', $notification->id) }}" method="POST" class="inline-block m-0">
                            @csrf
                            <button type="submit"
...
<style>
    ...
    .animate-pulse {
        animation: pulseOutline 1.4s infinite;
    }
</style>
@endsection