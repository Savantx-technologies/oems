                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <div class="font-bold text-gray-800 text-lg truncate w-48 flex items-center gap-2">
                                    ...
                                    <i class="bi bi-mic-fill text-gray-400"
...
<script>
            ...
            return {
                ...
                async startCamera(studentId) {
                    ...
                    this.viewerSessionId =
                        `admin_${studentId}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    this.loading = false;
                    ...
                    this.pc.onicecandidate = (event) => {
                        if (event.candidate) {
                            fetch(`/admin/stream/${this.streamId}/signal`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    ...
                    try {
                        // 1. Request a stream
                        const reqRes = await fetch(`/admin/attempts/${studentId}/request-stream`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        ...
                        this.iceInterval = setInterval(async () => {
                            ...
                            const pollRes = await fetch(`/admin/stream/${this.streamId}/poll`);
                            ...
                            if (pollData.offer && !this.pc.currentRemoteDescription) {
                                ...
                                await fetch(`/admin/stream/${this.streamId}/signal`, {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
